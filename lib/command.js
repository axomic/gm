...
module.exports = function (proto) {
  ...
  proto._spawn = function _spawn (args, bufferOutput, callback) {
    ...
    var stdioOptions = this.compositeStream ? 
                        {stdio:[null, null, null, 'pipe'] }:
                        {};


    var proc = spawn(bin, args, stdioOptions)
     ...
     proc.stderr.on('data', function(data){
        console.log('gm: proc StdErr', data);
     });

    debug(cmd);
    if (self.sourceBuffer) {
        proc.stdio[0].write(this.sourceBuffer);
        proc.stdio[0].end();
      ...
      if(this.compositeStream){
        proc.stdio[3].on('error', function(err){
            console.log('stdio3 error', err);
        });
        this.compositeStream.pipe(proc.stdio[3]);
      }
      // bufferStream
